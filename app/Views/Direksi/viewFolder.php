    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2 text-xl font-semibold text-gray-800">
            <nav class="text-sm font-medium text-gray-500">
                ...
                // NEW: Check for Supervisor Folder context first
                if (isset($isManagerFolder) && $isManagerFolder) {
                    $rootUrl = site_url('direksi/dokumen-manager'); // Adapt this URL
                    $rootName = 'Dokumen Manager'; // New root name
                } else if (isset($isSupervisorFolder) && $isSupervisorFolder) {
                    $rootUrl = site_url('direksi/dokumen-supervisor'); // Adapt this URL
                    $rootName = 'Dokumen Supervisor';
                } else if (isset($isStaffFolder) && $isStaffFolder) {
                    $rootUrl = site_url('direksi/dokumen-staff'); // Adapt this URL
                    $rootName = 'Dokumen Staff';
                } else {
                    $rootUrl = site_url('direksi/dokumen-direksi');
                ...
                <?php if (!empty($breadcrumbs)): ?>
                    <?php
                    $startIndex = 0;
                    // Adjust startIndex based on the detected root
                    ...
                    <?php for ($i = $startIndex; $i < $totalBreadcrumbs; $i++): ?>
                        ...
                        <?php if ($i < $totalBreadcrumbs - 1): // If not the last breadcrumb, make it clickable and blue ?>
                            ...
                            if (isset($isStaffFolder) && $isStaffFolder) {
                                $breadcrumbLink = site_url('direksi/view-staff-folder/' . $breadcrumb['id']);
                            } elseif (isset($isSupervisorFolder) && $isSupervisorFolder) {
                                // NEW: Link for Supervisor subfolders
                                $breadcrumbLink = site_url('direksi/view-supervisor-folder/' . $breadcrumb['id']); // You need this new route
                            } elseif (isset($isManagerFolder) && $isManagerFolder) {
                                // NEW: Link for Manager subfolders
                                $breadcrumbLink = site_url('direksi/view-manager-folder/' . $breadcrumb['id']); // You need this new route
                            } else {
                                $breadcrumbLink = site_url('direksi/view-folder/' . $breadcrumb['id']);
                            }
                            ?>
                        ...
                        <?php foreach ($subFolders as $subFolder): ?>
                            <tr class="hover:bg-gray-50 item-with-context-menu"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    $folderViewLink = '';
                                    if (isset($isStaffFolder) && $isStaffFolder) {
                                        $folderViewLink = base_url('direksi/view-staff-folder/' . $subFolder['id']);
                                    } elseif (isset($isSupervisorFolder) && $isSupervisorFolder) {
                                        // NEW: Link for Supervisor subfolders
                                        $folderViewLink = base_url('direksi/view-supervisor-folder/' . $subFolder['id']); // Needs to be defined in your routes
                                    } elseif (isset($isManagerFolder) && $isManagerFolder) {
                                        // NEW: Link for Manager subfolders
                                        $folderViewLink = base_url('direksi/view-manager-folder/' . $subFolder['id']); // Needs to be defined in your routes
                                    } else {
                                        $folderViewLink = base_url('direksi/view-folder/' . $subFolder['id']);
                                    }
                                    ?>
                                    <a href="<?= $folderViewLink ?>" class="block h-full w-full text-sm text-gray-900 hover:text-blue-700 hover:underline">
                    ...
                    <?php if (!empty($filesInFolder)): ?>
                        <?php foreach ($filesInFolder as $file): ?>
                            <tr class="hover:bg-gray-50 item-with-context-menu"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="<?= base_url('manager/view-file/' . $file['id']) ?>" target="_blank" class="block h-full w-full text-sm text-gray-900 hover:text-blue-700 hover:underline">
                                        <div class="flex items-center">
                                            <?php
