...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const searchApiUrl = '<?= base_url('direksi/searchStaff') ?>';
                    ...
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            data.forEach(item => {
                                ...
                                let url = '#';
                                if (item.type === 'folder') {
                                    url = `<?= base_url('direksi/view-staff-folder/') ?>${item.id}`;
                                } else {
                                    url = item.folder_id ? `<?= base_url('direksi/view-staff-folder/') ?>${item.folder_id}` : `<?= base_url('staff/dokumen-staff') ?>`;
                                }
                                a.href = url;
