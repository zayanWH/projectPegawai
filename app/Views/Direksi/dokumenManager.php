                        ...
                        <?php foreach ($personalFolders as $folder): ?>
                            <tr class="hover:bg-gray-50">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="<?= base_url('direksi/view-manager-folder/' . $folder['id']) ?>"
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const searchApiUrl = '<?= base_url('direksi/searchManager') ?>';
                    ...
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            data.forEach(item => {
                                ...
                                let url = '#';
                                if (item.type === 'folder') {
                                    url = `<?= base_url('direksi/view-manager-folder/') ?>${item.id}`;
                                } else {
                                    url = item.folder_id ? `<?= base_url('direksi/view-manager-folder/') ?>${item.folder_id}` : `<?= base_url('staff/dokumen-staff') ?>`;
                                }
                                a.href = url;
