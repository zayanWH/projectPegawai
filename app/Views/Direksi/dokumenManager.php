       ...
       document.addEventListener('DOMContentLoaded', function() {
            ...
            if (searchInput) {
                searchInput.addEventListener('input', function() {
                    ...
                    fetch('<?= site_url('direksi/searchManager') ?>', {
                            method: 'POST',
                            headers: {
                            ...
                            if (data.length > 0) {
                                data.forEach(item => {
                                    ...
                                    if (item.type === 'folder') {
                                        ...
                                        url = `<?= site_url('direksi/view-manager-folder/') ?>${item.id}`;
                                    } else if (item.type === 'file') {
                                        if (item.folder_id) {
                                            // File ada dalam folder
                                            url = `<?= site_url('direksi/view-manager-folder/') ?>${item.folder_id}`;
                                        } else {
                                            // File tanpa folder (orphan files)
                                            url = `<?= site_url('direksi/dokumen-manager') ?>`;
                                        }
                                    }
