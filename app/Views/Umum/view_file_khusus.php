...
<body class="bg-gray-100 flex flex-col min-h-screen">
    <header class="fixed top-0 left-0 right-0 bg-white shadow-md z-10 h-16 flex items-center justify-between px-6">
        <div class="flex items-center space-x-4 min-w-0">
            <button id="backButton" title="Kembali" aria-label="Kembali"
        ...
        <div class="flex items-center space-x-4">
            <a href="<?= site_url('staff/download-file/' . $fileId) ?>" id="downloadButtonHeader" title="Unduh File"
                aria-label="Unduh File" class="text-gray-600 hover:text-gray-900" download>
                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                    xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <main class="flex-grow flex flex-col items-center justify-center pt-20 pb-10 px-4">
        <div class="bg-white p-8 rounded-lg shadow-xl max-w-2xl w-full text-center">
            ...
            <p class="text-lg text-gray-700 mb-6">
                Browser tidak dapat menampilkan file jenis
                *<?= esc(strtoupper(pathinfo($fileName, PATHINFO_EXTENSION))) ?>* secara langsung. Untuk melihat
                kontennya, Anda perlu mengunduhnya.
            </p>
            ...
            <div class="bg-gray-50 p-6 rounded-md border border-gray-200 mb-8 text-left">
                ...
                <p class="text-gray-700 mb-2"><strong>Ukuran:</strong>
                    <?= number_format($file['file_size'] / 1024, 2) ?> KB</p>
                <p class="text-gray-700 mb-2"><strong>Tanggal Unggah:</strong>
                    <?= date('d M Y, H:i', strtotime($file['created_at'])) ?></p>
                <?php if (isset($file['uploaded_by_username'])): // Asumsi Anda punya kolom ini atau ambil dari relasi ?>
                    <p class="text-gray-700 mb-2"><strong>Diunggah Oleh:</strong> <?= esc($file['uploaded_by_username']) ?>
                    </p>
                <?php endif; ?>
            ...
            <p class="text-md text-gray-600 mb-8">
                *Untuk mengakses isi file, silakan klik tombol "Unduh File" di bawah ini.* Setelah diunduh, Anda dapat
                membukanya dengan aplikasi yang sesuai (seperti Microsoft Word, Excel, atau PowerPoint) di perangkat
                Anda.
            </p>

            <a href="<?= base_url('hrd/file/download/' . $fileId) ?>"
                class="bg-blue-600 text-white px-6 py-3 rounded-lg flex items-center justify-center font-semibold transition-all duration-300 transform hover:scale-105">
