                    ...
                    <?php foreach ($personalFolders as $folder): ?>
                        <tr class="hover:bg-gray-50" data-folder-id="<?= esc($folder['id']) ?>"
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <a href="<?= base_url('supervisor/view-staff-folder/' . $folder['id']) ?>"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (searchInput) {
                searchInput.addEventListener('input', function() {
                    ...
                    fetch('<?= site_url('supervisor/searchStaff') ?>', {
                            method: 'POST',
                            headers: {
                            ...
                            if (data.length > 0) {
                                data.forEach(item => {
                                    ...
                                    if (item.type === 'folder') {
                                        ...
                                        url = `<?= site_url('supervisor/view-staff-folder/') ?>${item.id}`;
                                    } else if (item.type === 'file') {
                                        if (item.folder_id) {
                                            // File ada dalam folder
                                            url = `<?= site_url('supervisor/view-staff-folder/') ?>${item.folder_id}`;
                                        } else {
                                            // File tanpa folder (orphan files)
                                            url = `<?= site_url('supervisor/dokumen-staff') ?>`;
                                        }
                                    }
