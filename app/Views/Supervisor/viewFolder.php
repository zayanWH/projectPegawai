                ...
                <?php if (!empty($breadcrumbs)): ?>
                    ...
                    <?php for ($i = $startIndex; $i < count($breadcrumbs); $i++): ?>
                        ...
                        <?php if ($breadcrumb['id'] !== null && isset($breadcrumb['url'])): ?>
                            <a href="<?= $breadcrumb['url'] ?>" class="text-blue-600 hover:text-blue-800"><?= esc($breadcrumb['name']) ?></a>
                        <?php else: ?>
                        ...
                        <?php foreach ($subFolders as $subFolder): ?>
                            <tr class="hover:bg-gray-50 item-with-context-menu"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="<?= (isset($isStaffFolder) && $isStaffFolder) ? base_url('supervisor/view-staff-folder/' . $subFolder['id']) : base_url('supervisor/folder/' . $subFolder['id']) ?>" class="block h-full w-full text-sm text-gray-900 hover:text-blue-700 hover:underline">
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (submitCreateFolder) {
            submitCreateFolder.addEventListener('click', function() {
                ...
                fetch(<?= site_url('supervisor/create-folder') ?>, {
                    method: 'POST',
                    headers: {
        ...
        if (submitUploadFile) {
            submitUploadFile.addEventListener('click', function() {
                ...
                fetch(<?= site_url('supervisor/upload-file') ?>, {
                    method: 'POST',
                    body: formData,
