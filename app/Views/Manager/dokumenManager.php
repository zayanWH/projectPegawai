    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                <h1 class="text-2xl font-semibold text-gray-800">Dokumen Manager</h1>
            </div>
            <div class="flex items-center space-x-4">
                <div class="relative">
...
<div class="hidden md:block">
    <div class="relative inline-block text-left mb-6">
        ...
        <div id="dropdownMenu"
            class="absolute z-10 mt-2 w-48 bg-white border border-gray-200 rounded-lg shadow-lg hidden opacity-0 invisible scale-95 transition-all duration-200 ease-out origin-top-left">
            <a href="#" id="openCreateFolder" class="block px-4 py-2 text-gray-700 hover:bg-gray-100">📁 Buat Folder</a>
            <a href="#" id="openUploadFolder" class="block px-4 py-2 text-gray-700 hover:bg-gray-100">📂 Upload
                Folder</a>
    ...
    <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md transition-all duration-300 ease-in-out">
        ...
        <div class="relative mb-4">
            <select id="folderType"
                class="w-full border rounded-lg px-3 py-2 pr-10 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        ...
        <div id="accessRolesContainer" class="mb-4 hidden">
            ...
            <div class="grid grid-cols-2 gap-2">
                <label><input type="checkbox" name="accessRoles[]" value="6" class="mr-2"> Staff</label>
                <label><input type="checkbox" name="accessRoles[]" value="4" class="mr-2"> Manager</label>
                <label><input type="checkbox" name="accessRoles[]" value="5" class="mr-2"> Supervisor</label>
                <label><input type="checkbox" name="accessRoles[]" value="3" class="mr-2"> Direksi</label>
            </div>
        </div>

        <label class="block text-sm font-medium mb-1">Akses</label>
        <div class="relative mb-4">
            <select id="folderAccess"
                class="w-full border rounded-lg px-3 py-2 pr-10 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        ...
        <input type="text" id="folderName" placeholder="Masukan nama folder"
            class="w-full border rounded-lg px-3 py-2 mb-4 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button id="createFolderBtn"
                class="px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700">Buat</button>
        </div>
    </div>
</div>
...
<div id="modalUploadFile"
    ...
    <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md transition-all duration-300 ease-in-out">
        ...
        <div class="mb-4">
            <label for="fileDescription" class="block text-sm font-medium text-gray-700 mb-1">Deskripsi
                (Opsional)</label>
            <textarea id="fileDescription" rows="3"
                class="w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                placeholder="Tambahkan deskripsi singkat tentang file ini..."></textarea>
        </div>
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button id="uploadFileBtn"
                class="px-4 py-2 rounded-lg bg-blue-600 text-white font-semibold hover:bg-blue-700">Unggah</button>
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const accessRolesContainer = document.querySelectorAll('#accessRolesContainer');
        ...
        function resetCreateFolderForm() {
            ...
            accessRolesContainer.forEach(el => el.classList.add('hidden'));
        ...
        if (openUploadFolder && folderUploadInput) {
            ...
            folderUploadInput.addEventListener('change', async function (e) {
                ...
                for (let i = 0; i < files.length; i++) {
                    ...
                    if (window.currentFolderId && window.currentFolderId > 0) {
                        formData.append('folder_id', window.currentFolderId);
                    }
                    formData.append('uploader_id', window.currentUserId);

                    try {
                        const response = await fetch('<?= base_url('manager/upload-file') ?>', {
                            method: 'POST',
                            body: formData,
                            headers: {
        ...
        if (folderTypeSelect && accessRolesContainer) {
            folderTypeSelect.addEventListener('change', function () {
                ...
                if (this.value === 'shared') {
                    accessRolesContainer.forEach(el => el.classList.remove('hidden'));
                } else {
                    folderAccessSelect.value = '';
                }
            });
        ...
        if (uploadFileBtn && fileInput) {
            uploadFileBtn.addEventListener('click', function () {
                ...
                fetch('<?= base_url('manager/upload-file') ?>', {
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    },
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const searchApiUrl = '<?= base_url('manager/search') ?>';
                    ...
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            data.forEach(item => {
                                ...
                                let url = '#';
                                if (item.type === 'folder') {
                                    url = `<?= base_url('manager/view-folder/') ?>${item.id}`;
                                } else {
                                    url = item.folder_id ? `<?= base_url('manager/view-folder/') ?>${item.folder_id}` : `<?= base_url('staff/dokumen-staff') ?>`;
                                }
                                a.href = url;
