    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center space-x-2 text-xl font-semibold text-gray-800">
            <nav class="text-sm font-medium text-gray-500">
                ...
                if (isset($isSupervisorFolder) && $isSupervisorFolder) {
                    $rootUrl = site_url('manager/dokumen-supervisor'); // Adapt this URL
                    $rootName = 'Dokumen Supervisor'; // New root name
                } else if (isset($isStaffFolder) && $isStaffFolder) {
                    $rootUrl = site_url('manager/dokumen-staff'); // Adapt this URL
                    $rootName = 'Dokumen Staff';
                } else {
                    $rootUrl = site_url('manager/dokumen-manager');
                    ...
                    <?php for ($i = $startIndex; $i < $totalBreadcrumbs; $i++): ?>
                        ...
                        <?php if ($i < $totalBreadcrumbs - 1): // If not the last breadcrumb, make it clickable and blue ?>
                            ...
                            if (isset($isStaffFolder) && $isStaffFolder) {
                                $breadcrumbLink = site_url('manager/view-staff-folder/' . $breadcrumb['id']);
                            } elseif (isset($isSupervisorFolder) && $isSupervisorFolder) {
                                // NEW: Link for Supervisor subfolders
                                $breadcrumbLink = site_url('manager/view-supervisor-folder/' . $breadcrumb['id']); // You need this new route
                            } else {
                                $breadcrumbLink = site_url('manager/view-folder/' . $breadcrumb['id']);
                            }
                            ?>
                        ...
                        <?php foreach ($subFolders as $subFolder): ?>
                            <tr class="hover:bg-gray-50 item-with-context-menu"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    ...
                                    $folderViewLink = '';
                                    if (isset($isStaffFolder) && $isStaffFolder) {
                                        $folderViewLink = base_url('manager/view-staff-folder/' . $subFolder['id']);
                                    } elseif (isset($isSupervisorFolder) && $isSupervisorFolder) {
                                        // NEW: Link for Supervisor subfolders
                                        $folderViewLink = base_url('manager/view-supervisor-folder/' . $subFolder['id']); // Needs to be defined in your routes
                                    } else {
                                        $folderViewLink = base_url('manager/view-folder/' . $subFolder['id']);
                                    }
                                    ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (submitCreateFolder) {
            submitCreateFolder.addEventListener('click', function() {
                ...
                fetch(`<?= site_url('manager/create-folder') ?>`, {
                    method: 'POST',
                    headers: {
