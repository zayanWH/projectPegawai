    ...
    <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
        ...
        <div class="relative inline-block text-left mb-6">
            ...
            <div id="dropdownMenu"
                ...
                <a href="#" id="openCreateFolder" class="block px-4 py-2 text-gray-700 hover:bg-gray-100">📁 Buat
                    Folder</a>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (dropdownButtonMobile && dropdownMenuMobile) {
            dropdownButtonMobile.addEventListener('click', function (e) {
                ...
                // Toggle dropdown "Baru"
                const isVisible = !dropdownMenuMobile.classList.contains('hidden'); // Cek apakah tidak hidden
                if (isVisible) {
        ...
        if (openCreateFolderMobile && modalCreateFolder) {
            openCreateFolderMobile.addEventListener('click', function (e) {
                e.preventDefault();
                hideDropdown(dropdownMenuMobile); // Tutup dropdown "Baru"
                resetCreateFolderForm(); // Reset form sebelum membuka
                showModal(modalCreateFolder); // Tampilkan modal folder
            });
        }
        ...
        if (openUploadFolder && folderUploadInput) {
            ...
            folderUploadInput.addEventListener('change', async function (e) {
                ...
                for (let i = 0; i < files.length; i++) {
                    ...
                    try {
                        const response = await fetch('<?= base_url('manager/upload-file') ?>', { // Mengarah ke Manager controller
                            method: 'POST',
                            body: formData,
                            headers: {
        ...
        if (uploadFileBtn && fileInput) {
            uploadFileBtn.addEventListener('click', function () {
                ...
                fetch('<?= base_url('manager/upload-file') ?>', { // Mengarah ke Manager controller
                    method: 'POST',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    },
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', function () {
                ...
                fetch('<?= site_url('hrd/searchManager') ?>', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            data.forEach(item => {
                                ...
                                let url = '#';
                                if (item.type === 'folder') {
                                    url = `<?= site_url('hrd/view-manager-folder/') ?>${item.id}`;
                                } else {
                                    if (item.folder_id) {
                                        url = `<?= site_url('hrd/view-manager-folder/') ?>${item.folder_id}`;
                                    } else {
                                        url = `<?= site_url('hrd/dokumen-manager') ?>`;
                                    }
                                }
                                a.href = url;
