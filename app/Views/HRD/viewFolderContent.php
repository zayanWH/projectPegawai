        ...
        <div class="flex items-center space-x-2 text-xl font-semibold text-gray-800">
            ...
            <?php if (!empty($breadcrumbs)): ?>
                <?php foreach ($breadcrumbs as $breadcrumb): ?>
                    ...
                    <a href="<?= site_url('hrd/view-staff-folder/' . $breadcrumb['id']) ?>" class="text-blue-600 hover:text-blue-800"><?= esc($breadcrumb['name']) ?></a>
                <?php endforeach; ?>
            <?php endif; ?>
        </div>
...
<div id="modalUploadFile" class="fixed inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-sm hidden">
    <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md transition-all duration-300 ease-in-out">
        ...
        <div class="flex justify-end space-x-4">
            ...
            <button id="submitUploadFile" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md">Upload</button>
        </div>
    </div>
</div>
...
<div id="modalCreateFolder" class="fixed inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-sm hidden">
    <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md transition-all duration-300 ease-in-out">
        ...
        <input type="text" id="newFolderNameInput" placeholder="Masukan nama folder" class="w-full border rounded-lg px-3 py-2 mb-4">
        <div class="flex justify-end space-x-4">
            <button id="cancelCreateFolderModal" class="text-blue-500 hover:text-blue-700">Batal</button>
            <button id="submitCreateFolder" class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md">Buat</button>
        </div>
    </div>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-blue-600">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-white uppercase tracking-wider">Tanggal Dibuat / Diunggah</th>
                    ...
                    <?php if (!empty($subFolders)): ?>
                        <?php foreach ($subFolders as $subFolder): ?>
                            <tr class="hover:bg-gray-50 item-with-context-menu"
                                ...
                                data-item-id="<?= esc($subFolder['id']) ?>"
                                data-item-name="<?= esc($subFolder['name']) ?>"
                                data-folder-id="<?= esc($subFolder['id']) ?>"
                                data-folder-name="<?= esc($subFolder['name']) ?>"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="<?= site_url('hrd/view-staff-folder/' . $subFolder['id']) ?>" class="block h-full w-full text-sm text-gray-900 hover:text-blue-700 hover:underline">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <button onclick="showFloatingMenu(event, 'folder', '<?= esc($subFolder['id']) ?>', '<?= esc($subFolder['name']) ?>')" class="text-blue-600 hover:text-blue-900">⋮</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
                    ...
                    <?php if (!empty($filesInFolder)): ?>
                        <?php foreach ($filesInFolder as $file): ?>
                            <tr class="hover:bg-gray-50 item-with-context-menu"
                                ...
                                data-file-size="<?= esc($file['file_size']) ?>"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="<?= site_url('hrd/file/view/' . $file['id']) ?>" target="_blank" class="block h-full w-full text-sm text-gray-900 hover:text-blue-700 hover:underline">
                                        <div class="flex items-center">
                                            <?php
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <button onclick="showFloatingMenu(event, 'file', '<?= esc($file['id']) ?>', '<?= esc($file['file_name']) ?>')" class="text-blue-600 hover:text-blue-900">⋮</button>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php endif; ?>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (submitUploadFile) {
            submitUploadFile.addEventListener('click', function() {
                ...
                // ✨ PENTING: Ubah URL ke rute HRD ✨
                fetch(`<?= site_url('hrd/upload-file') ?>`, {
                    method: 'POST',
                    body: formData,
