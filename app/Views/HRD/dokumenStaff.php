...
<div id="modalUploadFile" class="fixed inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-sm hidden">
    <div class="bg-white p-6 rounded-lg shadow-xl w-full max-w-md transition-all duration-300 ease-in-out">
        ...
        <textarea id="fileDescription" class="w-full border rounded-lg px-3 py-2 mb-4" rows="3" placeholder="Tambahkan deskripsi file..."></textarea>
    ...
    <?php if (!empty($personalFolders)): ?>
        <?php foreach ($personalFolders as $folder): ?>
            <tr class="hover:bg-gray-50"
                ...
                data-folder-is-shared="<?= esc($folder['is_shared'] ?? 0) ?>" // Menambahkan fallback untuk is_shared
                data-folder-shared-type="<?= esc($folder['shared_type'] ?? '') ?>" // Menambahkan fallback untuk shared_type
                data-folder-owner-id="<?= esc($folder['owner_id']) ?>"
                data-folder-owner-name="<?= esc($folder['owner_display'] ?? $folder['owner_name'] ?? 'Unknown') ?>" // Menambahkan fallback untuk owner_display dan owner_name
                data-folder-created-at="<?= esc($folder['created_at']) ?>"
                data-folder-updated-at="<?= esc($folder['updated_at']) ?>"
                data-folder-path="<?= esc($folder['path'] ?? $folder['name']) ?>"> <td class="px-6 py-4 whitespace-nowrap">
                    <a href="<?= base_url('staff/folder/' . $folder['id']) ?>" class="block h-full w-full text-sm text-gray-900 hover:text-blue-700 hover:underline">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                <?php foreach ($orphanFiles as $file): ?>
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                            <a href="<?= base_url('staff/download-file/' . $file['id']) ?>" class="text-blue-600 hover:text-blue-900 mr-2">Unduh</a>
                            </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
