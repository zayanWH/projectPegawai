                        ...
                        <?php foreach ($personalFolders as $folder): ?>
                            <tr class="hover:bg-gray-50" data-folder-id="<?= esc($folder['id']) ?>"
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <a href="<?= base_url('hrd/view-direksi-folder/' . $folder['id']) ?>"
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (createFolderBtn && folderNameInput && folderTypeSelect && folderAccessSelect) {
            createFolderBtn.addEventListener('click', function () {
                ...
                fetch('<?= base_url('hrd/create-folder-direksi') ?>', {
                    method: 'POST',
                    headers: {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', function () {
                ...
                fetch('<?= site_url('hrd/searchDireksi') ?>', {
                    method: 'POST',
                    headers: {
                    ...
                    .then(data => {
                        ...
                        if (data.length > 0) {
                            data.forEach(item => {
                                ...
                                let url = '#';
                                if (item.type === 'folder') {
                                    url = `<?= site_url('hrd/view-direksi-folder/') ?>${item.id}`;
                                } else {
                                    if (item.folder_id) {
                                        url = `<?= site_url('hrd/view-direksi-folder/') ?>${item.folder_id}`;
                                    } else {
                                        url = `<?= site_url('hrd/dokumen-direksi') ?>`;
                                    }
                                }
                                a.href = url;
